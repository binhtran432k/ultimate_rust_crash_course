...
fn expensive_sum(v: Vec<i32>) -> i32 {
    ...
    v.iter().filter(|&x| x & 1 == 0).map(|&x| x * x).sum()
}
...
fn main() {
    ...
    tx.send("Main Thread: Hello 1").unwrap();
    tx.send("Main Thread: Hello 2").unwrap();
    tx.send("Main Thread: Hello 3").unwrap();
    tx.send("Main Thread: Hello 4").unwrap();
    tx.send("Main Thread: Hello 5").unwrap();
